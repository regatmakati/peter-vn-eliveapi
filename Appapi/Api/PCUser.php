...
class Api_PCUser extends PhalApi_Api {
    ...
    public function getRules() {
		return array(
			...
			'getLiverecord' => array(
                'uid' => array('name' => 'uid', 'type' => 'int', 'min' => 1, 'require' => true, 'desc' => '用户ID'),
                'endtime' => array('name' => 'endtime', 'type' => 'int', 'min' => 1, 'require' => true, 'desc' => '结束时间'),
                'starttime' => array('name' => 'starttime', 'type' => 'int', 'min' => 1, 'require' => true, 'desc' => '开始时间'),
                'uid' => array('name' => 'uid', 'type' => 'int', 'min' => 1, 'require' => true, 'desc' => '用户ID'),
                'token' => array('name' => 'token', 'type' => 'string', 'min' => 1, 'require' => true, 'desc' => '用户token'),
                'p' => array('name' => 'p', 'type' => 'int', 'min' => 1, 'default'=>1,'desc' => '页数'),
			),
	...
	public function updateFields() {
	    $a = ['user_nicename' => 'ddd', 'sex' => 1, 'signature' => '我是签名', 'birthday' => '0901'];
...
//			}
	...
	public function getFollowsList() {
		...
		$domain = new Domain_User();
		$res['list'] = $domain->getPCFollowsList($this->uid,$this->p,$this->limit);
        $res['count'] = $domain->getPCFollowsCount($this->uid);
		$rs['info']=$res;
		return $rs;
	}		
	...
	public function getFansList() {
        ...
        $domain = new Domain_User();
		$res['list'] = $domain->getPCFansList($this->uid,$this->p,$this->limit);
	    $res['count'] = $domain->getPCFansCount($this->uid);
		$rs['info']=$res;
		return $rs;
	}	
	...
	public function getLiverecord() {
        ...
        $domain = new Domain_User();
		$info = $domain->getPCLiverecord($param, $this->uid,$this->p);
    ...
    */		
     ...
     * @return array info[0]['sublive'] 订阅正在直播列表
     * @return string info[0]['sublive'][].user_nicename 用户名
     * @return string info[0]['sublive'][].avatar 用户头像
     * @return string info[0]['sublive'][].avatar_thumb 用户头像缩略
     * @return string info[0]['sublive'][].islive 0未直播  1在直播
     * @return string info[0]['sublive'][].thumb  直播图片
     * @return string info[0]['sublive'][].title  直播标题
     * @return array info[0]['noSublive'] 订阅未直播列表
     * @return string info[0]['noSublive'][].user_nicename 用户名
     * @return string info[0]['noSublive'][].avatar 用户头像
     * @return string info[0]['noSublive'][].avatar_thumb 用户头像缩略
     * @return string msg 提示信息
     */
    public function getSubscribe() {
        ...
        $domain = new Domain_User();
        $info = $domain->PCgetSubscribes($this->uid);

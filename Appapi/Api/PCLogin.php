...
class Api_PCLogin extends PhalApi_Api {
    ...
    public function userLogin() {
		...
		if($info==1001){
			...
			$rs['code'] = 1003;
            $rs['info'][0]['ban_tip'] ="该账号已被禁用";
            $rs['msg'] = '该账号已被禁用';
            return $rs;	
		}else if($info==1004){
            $rs['code'] = 1004;
            $rs['info'][0]['ban_tip'] ="该账号已注销";
            $rs['msg'] = '该账号已注销';
            return $rs; 
        }
   ...
   /**
    ...
    public function userReg() {
        ...
        $sendcode = DI()->notorm->sendcode
            ->where('account=?',$user_login)
            ->select('addtime,content')
            ->order('id desc')
            ->fetchOne();
        if(!$sendcode && $sendcode['addtime'] + 300 < time()){
            $rs['code'] = 1001;
            $rs['msg'] = '请先获取验证码';
            return $rs;
        }
...
//        }
		...
		$domain = new Domain_Login();
		$info = $domain->userRegPc($user_login,$user_pass,$source);
...
//		}
    ...
    {
        $user_login=checkNull($this->user_login);
        $info = (new Domain_Login())->checkUserExists($user_login);
        if ($info === 1006) {
            return array(
                "code" => $info,
                "msg" => "该账号不存在!",

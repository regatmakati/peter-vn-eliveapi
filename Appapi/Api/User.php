...
class Api_User extends PhalApi_Api {
	...
	public function getRules() {
		return array(
            ...
            'getSubscribeList' => array(
                'uid' => array('name' => 'uid', 'type' => 'int', 'min' => 1, 'require' => true, 'desc' => '用户ID'),
                'limit' => array('name' => 'limit', 'type' => 'int', 'default'=>'20' ,'desc' => '每页记录数'),
                'p' => array('name' => 'p', 'type' => 'int', 'min' => 1, 'default'=>1,'desc' => '页数'),
            ),
			'getBlackList' => array(
				'uid' => array('name' => 'uid', 'type' => 'int', 'min' => 1, 'require' => true, 'desc' => '用户ID'),
	...
	public function getOrderrecord() {
		...
		$info=$domain->getOrderrecord($this->uid,$this->p);
	 ...
	 * @desc 用于获取用户金币记录
	 * @return int code 操作码，0表示成功
	 * @return array info 用户金币列表
	 * @return string msg 提示信息
	 */
	public function getCoinrecord() {
		$rs = array('code' => 0, 'msg' => '', 'info' => array());
		
		$domain = new Domain_User();
		$info=$domain->getCoinrecord($this->uid,$this->p);
	 ...
	 * @return string info[0].user_login 用户名
	...
	public function getBaseInfo() {
        ...
        $list1[]=array('id'=>'25','name'=>'我的消息','thumb'=>get_upload_path("image/personal/notice.png"),'href'=>'' );
		//if($shelves){
        ...
        $list2[]=array('id'=>'20','name'=>'房间管理','thumb'=>get_upload_path("image/personal/rooms.png") ,'href'=>'');
		//if($shelves){
		...
		$list3[]=array('id'=>'16','name'=>'版本更新','thumb'=>get_upload_path("image/personal/version.png") ,'href'=>'' );
		if($configpri['service_switch']){
			$list3[]=array('id'=>'0','name'=>'联系我们','thumb'=>get_upload_path("image/personal/contact.png") ,'href'=>$service_url);			
		}


		$list3[]=array('id'=>'0','name'=>'意见反馈','thumb'=>get_upload_path("image/personal/feedback.png") ,'href'=>get_upload_path('/Appapi/feedback/index'));

		
		if($agent_switch){
			$list2[]=array('id'=>'8','name'=>'邀请奖励','thumb'=>get_upload_path("image/personal/agent.png") ,'href'=>get_upload_path("/Appapi/Agent/index"));
		}
		...
		$list3[]=array('id'=>'13','name'=>'个人设置','thumb'=>get_upload_path("image/personal/sets.png") ,'href'=>'');
	...
	public function updateAvatar() {
		...
		if($uptype==1){
		   ...
		   // DI()->ucloud->set('file_name', $this->uid);
            ...
            //华为
            $url = DI()->huawei->uploadFile($_FILES['file']['tmp_name'],'avatar');
	...
	public function updateFields() {
		...
		if(array_key_exists('user_nicename', $fields)){
            if (checkSensitiveWords($fields['user_nicename'])) {
                $rs['code'] = 1002;
                $rs['msg'] = '用户名不能包含违禁词';
                return $rs;
            }
		...
		if(array_key_exists('signature', $fields)){
            if (checkSensitiveWords($fields['signature'])) {
                $rs['code'] = 1002;
                $rs['msg'] = '签名不能包含违禁词';
                return $rs;
            }
...
//			}
	...
	public function getSubscribeList(){
        ...
        $domain = new Domain_User();
        $info['list'] = $domain->getSubscribeList($this->uid,$this->p,$this->limit);
        $info['count'] = $domain->getSubscribeCount($this->uid);
        $rs['info']=$info;
        return $rs;
    }
...
//        $info = $domain->getPerSetting();
		...
		$info[]=array('id'=>'0','name'=>'权限详情','thumb'=>'' ,'href'=>get_upload_path("/portal/page/index?id=46"));
...
//        $info[]=array('id'=>'16','name'=>'检查更新','thumb'=>'' ,'href'=>'');
     ...
     * @return int code 操作码，0表示成功
     * @return array info
     * @return string info[].islive 是否直播,0表示未直播，1表示已直播
     * @return string msg 提示信息
     */
    public function getSubscribe() {
        ...
        $domain = new Domain_User();
        $info = $domain->getSubscribes($this->uid,$this->p);

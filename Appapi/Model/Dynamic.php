...
class Model_Dynamic extends PhalApi_Model_NotORM {
	...
	public function getRecommendDynamics($uid,$p){
        ...
        $sql = "SELECT floor( RAND() * ((SELECT MAX(id) FROM cmf_dynamic)-(SELECT MIN(id) FROM cmf_dynamic)) + (SELECT MIN(id) FROM cmf_dynamic)) as rand";
        $data = DI()->notorm->example->queryAll($sql);
        $rand = $data[0]['rand']-$pnums;
...
//        ->order("recommend_val desc,recomend desc,addtime desc")
	...
	public function report($data) {
        ...
        $ReportNum=DI()->notorm->dynamic_report
            ->where("uid='{$data['uid']}' and dynamicid='{$data['dynamicid']}'")
            ->count();
        if($ReportNum>=1){
            return 1001;
        }

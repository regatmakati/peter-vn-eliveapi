...
class Model_Login extends PhalApi_Model_NotORM {
	...
	protected $fields='id,user_nicename,avatar,avatar_thumb,sex,signature,coin,consumption,votestotal,province,city,birthday,user_status,end_bantime,login_type,last_login_time,location,issuper';
    ...
    public function userLogin($user_login,$user_pass) {
		...
		$usertoken=DI()->notorm->user_token
            ->select('token,expire_time')
            ->where('user_id = ?', $info['id'])
            ->fetchOne();
        if($usertoken && $usertoken['expire_time'] >= time() && $usertoken['token']){
        ...
        setcaches("chatUserinfo_".$info['id'],['id'=>$info['id'],'user_nicename'=>$info['user_nicename'],'avatar'=>$info['avatar'],'avatar_thumb'=>$info['avatar_thumb'],'user_status'=>$info['user_status']]);
	...
	/* 会员注册 */
    public function userReg($user_login,$user_pass,$source,$type=1) {

		$user_pass=setPass($user_pass);
		if($type == 2){
		    $user_nickname = "web用户".substr($user_login,-4);
        }else{
		...
		$data=array(
			...
			'avatar_thumb' =>'image/avatar/default_thumb.jpg',
	...
	public function userFindPass($user_login,$user_pass){
		...
		$user_pass=setPass($user_pass);
        if($user_pass == $isexist['user_pass']){
            return 1008;
        }
    ...
    public function updateToken($uid,$token,$data=array()) {
        ...
        $redis->setex("chatToken_" . $uid, $expiretime-$nowtime, $token);
		return 1;
    }	
    ...
    /**
     * 查询用户手机好是否存在
     * @param string $user_login 用户手机号
     * @return int|array
     */
    public function checkUserExists($user_login)
    {
        $info=DI()->notorm->user
				->select('user_nicename')
                ->where(array(
                    "user_login" => $user_login,
                    "user_type" => $this->user_types['user'],
                ))
				->fetchOne();
